...
- **TUI named band presets**: when `bands.toml` is present in the current working directory, the `Bands` field loads its named presets alongside the built-in band sets and the `Custom…` checklist option.
